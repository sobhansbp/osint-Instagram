...
async function doLogin(){
  ...
  if(!val){ err.textContent = 'کد عبور را وارد کنید'; return; }
  ...
  if(hash === ADMIN_HASH){
    ...
    if(u){
      ...
      err.textContent = '❌ کد عبور نامعتبر است';
...
function enterApp(){
  ...
  chip.textContent = APP.isAdmin ? 'مدیریت' : (TAB_DEF[APP.role]?.name || 'کاربر');
  chip.className = 'user-chip' + (APP.isAdmin ? ' admin' : '');

  // settings
  APP.worker = S.get('cfg_worker') || '';

  buildTabs();
  // init each section (scripts define init fns)
...
function buildTabs(){
  ...
  bar.innerHTML = tabs.map(t=>{
    const d = TAB_DEF[t];
    const cls = 'tab' + (t==='admin'?' admin-tab':'');
    return `<button class="${cls}" data-tab="${t}" onclick="switchTab('${t}')">${d.icon} ${d.name}</button>`;
  }).join('');
  // hide tabs bar if only one tab
  bar.style.display = tabs.length<=1 ? 'none' : 'flex';
}
...
function switchTab(name){
  ...
  document.getElementById('sectionName').textContent = TAB_DEF[name]?.title || '';
  // notify section it became visible
...
function toast(msg){ try{ const d=document.createElement('div'); d.textContent=msg; d.style.cssText='position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#16202e;border:1px solid #2a3847;color:#dce5f0;padding:11px 20px;border-radius:12px;font-size:13px;z-index:999;font-family:Vazirmatn;box-shadow:0 8px 24px rgba(0,0,0,0.4)'; document.body.appendChild(d); setTimeout(()=>d.remove(),2200);}catch(e){} }

// —— Setup panel (shared, API fields admin-only) ——
function initSetup(){
  const p = document.getElementById('panel-setup');
  p.innerHTML = `
    <div class="card">
      <div class="card-title"><span class="ico">🌐</span> اتصال به Worker <span class="line"></span></div>
      <div style="display:flex;gap:8px;">
        <input type="text" id="workerUrl" placeholder="xxxxx.workers.dev" style="flex:1;">
      ...
      <div class="hint">آدرس Cloudflare Worker که همه درخواست‌ها از آن عبور می‌کند (دور زدن تحریم).</div>
    </div>
    ${APP.isAdmin ? `
    <div class="card">
      <div class="card-title"><span class="ico">🔐</span> کلیدهای API <span class="line"></span></div>
      <div class="sbox sbox-warn" style="margin-top:0;">کلیدهای Groq و ScrapeCreators به‌صورت امن داخل خود Worker ذخیره شده‌اند و در این صفحه نمایش داده نمی‌شوند. برای تغییر، کد Worker را در Cloudflare ویرایش کنید.</div>
    </div>` : ''}
  `;
  const w = S.get('cfg_worker'); if(w) document.getElementById('workerUrl').value = w;
}
function saveWorker(){
