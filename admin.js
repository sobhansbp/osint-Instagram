...
function renderAdminPanel(){
  ...
  p.innerHTML=`
    ...
    <div class="card accent-green">
      <div class="card-title"><span class="ico">📡</span> SYSTEM_STATUS // LIVE</div>
      <div id="adminLiveStatus" style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:12px;"></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button class="btn btn-sm" onclick="adminLoadReports()">🔄 LOAD_REPORTS</button>
        <button class="btn btn-sm" onclick="adminAISummary()">🤖 AI_SUMMARY</button>
      </div>
    ...
    <!-- نقشه فعالیت -->
    <div class="card">
      <div class="card-title"><span class="ico">🗓️</span> ACTIVITY_MAP // ۳۰ روز اخیر</div>
      <div id="adminHeatmap"></div>
    </div>

    <!-- آمار -->
    <div class="card">
      <div class="card-title"><span class="ico">📊</span> STATS</div>
      <div class="stats-row" id="adminStats">
        <div class="stat"><div class="stat-num" style="color:var(--dim);">—</div><div class="stat-label">LOADING</div></div>
      </div>
    </div>

    <div class="card">
      <div class="card-title"><span class="ico">📋</span> ACTIVITY_LOG</div>
      <div class="filter-row">
        <span class="filter-pill fa" id="af-all" onclick="adminFilter('all')">ALL</span>
        <span class="filter-pill" id="af-content" onclick="adminFilter('content')">CONTENT</span>
        <span class="filter-pill" id="af-monitor" onclick="adminFilter('monitor')">MONITOR</span>
        <span class="filter-pill" id="af-persona" onclick="adminFilter('persona')">PERSONA</span>
        <span class="filter-pill" id="af-hr" onclick="adminFilter('hr')">HR</span>
      </div>
      <div id="adminReports"><div class="empty">// LOAD_REPORTS_TO_VIEW</div></div>
    </div>

    <div class="card">
      <div class="card-title"><span class="ico">👤</span> OPERATORS</div>
      <div id="adminUsers"></div>
    </div>

    <div class="card">
      <div class="card-title"><span class="ico">🔑</span> ACCESS_CODES</div>
      <div class="hint" style="margin-bottom:10px;">// افزودن/حذف کد دسترسی</div>
      <input type="text" id="newPwCode" placeholder="XXXX-0000" style="margin-bottom:8px;direction:ltr;">
      ...
      <button class="btn btn-sm" style="width:100%;" onclick="adminAddPw()">[ ADD_CODE ]</button>
      <div id="adminPwList" style="margin-top:12px;"></div>
      <button class="btn btn-danger" style="width:100%;margin-top:10px;" onclick="adminResetPw()">[ RESET_DEFAULT ]</button>
    </div>

    <div class="card">
      <div class="card-title"><span class="ico">🗑️</span> DATA_MANAGEMENT</div>
      <button class="btn btn-danger" style="width:100%;margin-bottom:8px;" onclick="adminClearLocal()">[ CLEAR_LOCAL_STATS ]</button>
      <button class="btn btn-danger" style="width:100%;" onclick="adminClearCentral()">[ CLEAR_CENTRAL_KV ]</button>
    </div>
  `;
  adminRenderUsers();
  adminRenderPwList();
  adminRenderStatsLocal();
  adminRenderLiveStatus();
  adminRenderHeatmap();
}

function onShow_admin(){ if(APP.isAdmin){ adminRenderUsers(); adminRenderStatsLocal(); adminRenderLiveStatus(); adminRenderHeatmap(); } }

// —— وضعیت لحظه‌ای ——
function adminRenderLiveStatus(){
  const box=document.getElementById('adminLiveStatus'); if(!box)return;
  const stats=S.get('activity_stats')||{};
  const users=Object.keys(stats).length;
  const totalScans=Object.values(stats).reduce((a,v)=>a+v.scans,0);
  const hrAlerts=(S.get('hr_alerts')||[]).length;
  const workerOk=!!APP.worker;
  box.innerHTML=`
    <div style="background:var(--bg2);border:1px solid var(--border2);border-left:2px solid var(--green);padding:10px;">
      <div style="font-family:var(--font-mono);font-size:8px;color:var(--dim);letter-spacing:2px;">OPERATORS</div>
      <div style="font-family:var(--font-hud);font-size:22px;color:var(--green);">${users}</div>
    </div>
    <div style="background:var(--bg2);border:1px solid var(--border2);border-left:2px solid var(--amber);padding:10px;">
      <div style="font-family:var(--font-mono);font-size:8px;color:var(--dim);letter-spacing:2px;">OPERATIONS</div>
      <div style="font-family:var(--font-hud);font-size:22px;color:var(--amber);">${totalScans}</div>
    </div>
    <div style="background:var(--bg2);border:1px solid var(--border2);border-left:2px solid ${hrAlerts>0?'var(--red)':'var(--green)'};padding:10px;">
      <div style="font-family:var(--font-mono);font-size:8px;color:var(--dim);letter-spacing:2px;">ALERTS</div>
      <div style="font-family:var(--font-hud);font-size:22px;color:${hrAlerts>0?'var(--red)':'var(--green)'};">${hrAlerts}</div>
    </div>
    <div style="background:var(--bg2);border:1px solid var(--border2);border-left:2px solid ${workerOk?'var(--green)':'var(--red)'};padding:10px;">
      <div style="font-family:var(--font-mono);font-size:8px;color:var(--dim);letter-spacing:2px;">SERVER</div>
      <div style="font-family:var(--font-mono);font-size:11px;color:${workerOk?'var(--green)':'var(--red)'};margin-top:4px;">${workerOk?'ONLINE':'OFFLINE'}</div>
    </div>`;
}

// —— نقشه فعالیت (Heatmap) ——
function adminRenderHeatmap(){
  const box=document.getElementById('adminHeatmap'); if(!box)return;
  const stats=S.get('activity_stats')||{};

  // ساخت دیکشنری تاریخ → تعداد فعالیت
  const dateMap={};
  const hrHistory=S.get('hr_history')||[];
  hrHistory.forEach(r=>{ const d=new Date(r.ts).toISOString().split('T')[0]; dateMap[d]=(dateMap[d]||0)+r.urls.length; });
  const contentHistory=S.get('content_history')||[];
  contentHistory.forEach(r=>{ const d=new Date(r.ts).toISOString().split('T')[0]; dateMap[d]=(dateMap[d]||0)+1; });
  const monHistory=S.get('monitor_history')||[];
  monHistory.forEach(r=>{ const d=new Date(r.ts).toISOString().split('T')[0]; dateMap[d]=(dateMap[d]||0)+1; });
  ...
  for(let i=29;i>=0;i--){
    ...
    days.push({key,count:dateMap[key]||0,label:d.toLocaleDateString('fa-IR',{month:'short',day:'numeric'})});
  }

  const maxCount=Math.max(...days.map(d=>d.count),1);
  const cells=days.map(d=>{
    const intensity=d.count/maxCount;
    const bg=d.count===0?'var(--border2)':intensity<0.3?'rgba(0,255,65,0.2)':intensity<0.6?'rgba(0,255,65,0.5)':'rgba(0,255,65,0.9)';
    const isToday=d.key===new Date().toISOString().split('T')[0];
    return `<div title="${d.label}: ${d.count} فعالیت" style="width:100%;aspect-ratio:1;background:${bg};border:1px solid ${isToday?'var(--amber)':'var(--border2)'};cursor:default;transition:all 0.2s;" onmouseover="this.style.transform='scale(1.3)'" onmouseout="this.style.transform='scale(1)'"></div>`;
  }).join('');

  box.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(15,1fr);gap:3px;margin-bottom:8px;">${cells}</div>
    <div style="display:flex;align-items:center;gap:8px;font-family:var(--font-mono);font-size:9px;color:var(--dim);">
      <span>کم</span>
      <div style="display:flex;gap:2px;">
        ${['rgba(0,255,65,0.1)','rgba(0,255,65,0.3)','rgba(0,255,65,0.6)','rgba(0,255,65,0.9)'].map(c=>`<div style="width:10px;height:10px;background:${c};border:1px solid var(--border2);"></div>`).join('')}
      </div>
      <span>زیاد</span>
      <span style="margin-right:auto;color:var(--amber);">█ امروز</span>
    </div>`;
}
