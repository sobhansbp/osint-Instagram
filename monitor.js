// ============================================================
//  monitor.js — بخش رصد (نسخه ۵.۰)
//  موتور: Perplexity Sonar از لیارا (سرچ زنده داخلی)
//  + چت تحلیلی مستقیم
//  + بدون نیاز به Worker
// ============================================================
...
const LIARA_BASE = 'https://ai.liara.ir/api/6a231b42bebce2c5e031b891/v1';
const LIARA_KEY  = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXkiOiI2YTIzODRiOTk1M2NlZWViYmRiZThkNDQiLCJ0eXBlIjoiYWlfa2V5IiwiaWF0IjoxNzgwNzEyNjMzfQ.hCOuNYsm-iFOseXTUc4zIo_025jlQTYGNcg7_a-NYzs';
  ...
  p.innerHTML = `
    ...
    <div class="card">
      ...
      <div id="mon-tab-daily" style="display:${MONITOR.activeTab==='daily'?'block':'none'};">
        <div class="hint" style="margin-bottom:10px;">تحلیل اخبار روز اسرائیل با جستجوی زنده</div>
      ...
      <div id="mon-tab-chat" style="display:${MONITOR.activeTab==='chat'?'block':'none'};">
        ...
        <div id="monChatBox" style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:12px;min-height:200px;max-height:400px;overflow-y:auto;margin-bottom:10px;"></div>
...
// —— Perplexity Sonar از لیارا (مستقیم) ——
async function sonarChat(messages, opts={}){
  const payload={
    model: opts.model || 'perplexity/sonar',
    messages,
    temperature: opts.temperature ?? 0.4,
    max_tokens: opts.max_tokens ?? 2000,
  };

  const res = await fetch(`${LIARA_BASE}/chat/completions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${LIARA_KEY}`,
    },
    body: JSON.stringify(payload),
    signal: AbortSignal.timeout(90000),
  });

  if(!res.ok){ const err=await res.text(); throw new Error(`HTTP ${res.status}: ${err.substring(0,300)}`); }
...
const QUICK_TOOLS={
  breaking:{label:'⚡ اخبار فوری',prompt:'آخرین اخبار فوری امروز اسرائیل چیست؟'},
  trends:{label:'📈 روندهای داغ',prompt:'مهم‌ترین موضوعات داغ و ترند امروز در رسانه‌های اسرائیل؟'},
...
async function monQuickTool(type){ const t=QUICK_TOOLS[type]; if(!t)return; await monExecute(t.prompt,t.label,'brief'); }
async function monRunDaily(){ const d=document.getElementById('monDailyDepth').value; const p={brief:'مهم‌ترین اخبار امروز اسرائیل را خلاصه کن.',normal:'تحلیل جامع اخبار روز اسرائیل.',deep:'تحلیل عمیق اخبار روز اسرائیل با زمینه تاریخی.'}; await monExecute(p[d],'🌐 تحلیل اخبار روز',d); }
...
async function monExecute(query, label, depth){
  ['monDailyBtn','monTopicBtn','monProfileBtn','monCompareBtn'].forEach(id=>{
    const el=document.getElementById(id); if(el){el.disabled=true;el.innerHTML='<span class="spinner"></span> در حال جستجو...';}
  });
  const box=document.getElementById('monResult');
  box.innerHTML=`<div class="card"><div class="sbox sbox-load" style="margin:0;"><span class="spinner"></span> جستجوی زنده در رسانه‌های اسرائیل...</div></div>`;

  const now = new Date();
  const todayEn = now.toISOString().split('T')[0];
  const todayFa = now.toLocaleDateString('fa-IR');

  const sys=`تو تحلیلگر ارشد رسانه‌های اسرائیل هستی.
تاریخ دقیق امروز: ${todayEn} (${todayFa})
قانون مهم: فقط اخبار مربوط به تاریخ ${todayEn} یا حداکثر ۳ روز قبل از آن را بیاور. هر خبری قبل از این بازه را نادیده بگیر.
از جستجوی زنده وب استفاده کن و منابع معتبر اسرائیلی مثل Haaretz, Ynet, Times of Israel را اولویت بده.
پاسخ فارسی با این ساختار:
...
📌 منابع (با تاریخ):`;
  ...
  // تاریخ رو مستقیم توی query هم میذاریم
  const queryWithDate = `[امروز ${todayEn}] ${query}`;

  const maxT=depth==='brief'?1000:depth==='deep'?3000:2000;
  const model=depth==='deep'?'perplexity/sonar-pro':'perplexity/sonar';

  try{
    if(window.showTransmissionNoise) window.showTransmissionNoise();
    const raw=await sonarChat([{role:'system',content:sys},{role:'user',content:queryWithDate}],{model,max_tokens:maxT});
  ...
  ['monDailyBtn','monTopicBtn','monProfileBtn','monCompareBtn'].forEach(id=>{
    const el=document.getElementById(id); if(el){el.disabled=false;const lb={'monDailyBtn':'🌐 تحلیل اخبار روز','monTopicBtn':'🔍 جستجو','monProfileBtn':'👤 رصد','monCompareBtn':'⚖️ مقایسه'};el.innerHTML=lb[id]||'اجرا';}
  });
}
...
async function monSendChat(){
  ...
  inp.value='';
  const btn=document.getElementById('monChatBtn'); btn.disabled=true; btn.textContent='...';
  ...
  const sys=`تو تحلیلگر ارشد رسانه‌های اسرائیل هستی.
تاریخ دقیق امروز: ${today} (${todayFa})
قانون: فقط اخبار ${today} یا حداکثر ۳ روز قبل را بیاور. از جستجوی زنده وب استفاده کن. پاسخ فارسی روان.`;

  try{
    const msgs=[{role:'system',content:sys},...MONITOR.chatHistory.slice(-8)];
    // تاریخ رو به آخرین پیام کاربر هم اضافه کن
    if(msgs.length>1 && msgs[msgs.length-1].role==='user'){
      msgs[msgs.length-1]={role:'user',content:`[امروز ${today}] ${msgs[msgs.length-1].content}`};
    }
    const reply=await sonarChat(msgs,{model:'perplexity/sonar',temperature:0.6,max_tokens:1500});
