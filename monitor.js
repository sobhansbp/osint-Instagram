...
async function monExecute(query, label){
  ...
  try{
    const sys='تو تحلیلگر اطلاعاتی رسانه‌های اسرائیل هستی. فقط فارسی و مختصر پاسخ بده.';
    const usr=`${query}\n\nسه بخش کوتاه:\n📋 خلاصه:\n🔍 تحلیل:\n🎯 پیشنهاد اقدام:`;
    log('جستجوی زنده وب توسط هوش مصنوعی...','info');
    let raw;
    try{
      raw=await groqChat('monitor',[{role:'system',content:sys},{role:'user',content:usr}],{model:'groq/compound-mini',temperature:0.6,max_tokens:1200});
    }catch(err){
      if(/too_large|entity too|413|rate_limit|tokens per minute|TPM/i.test(err.message)){
        log('حجم زیاد بود، تلاش مجدد با درخواست کوچک‌تر...','gold');
        await new Promise(r=>setTimeout(r,2000));
        raw=await groqChat('monitor',[{role:'user',content:usr}],{model:'groq/compound-mini',temperature:0.6,max_tokens:900});
      } else throw err;
    }
    ...
    let msg=e.message;
    if(/too_large|413|tokens per minute|TPM|rate_limit/i.test(msg)) msg='حجم درخواست از حد مجاز تیر رایگان Groq (۶۰۰۰ توکن در دقیقه) بیشتر شد. چند لحظه صبر کنید و دوباره امتحان کنید، یا موضوع کوتاه‌تری جستجو کنید.';
    box.innerHTML=`<div class="sbox sbox-err">❌ ${esc(msg)}</div>`; log('خطا: '+e.message,'err');
  }
