...
async function monExecute(query, label, depth){
  ...
  const sys=`تو تحلیلگر ارشد رسانه‌های اسرائیل هستی. تاریخ امروز: ${today}.
از جستجوی زنده اینترنت استفاده کن و فقط اخبار امروز و چند روز اخیر را بیاور.
...
📌 منابع:`;
  ...
  try{
    const raw=await sonarChat([{role:'system',content:sys},{role:'user',content:query}],{model,max_tokens:maxT});
...
async function monSendChat(){
  ...
  const today=new Date().toISOString().split('T')[0];
  const sys=`تو تحلیلگر ارشد رسانه‌های اسرائیل هستی. تاریخ امروز: ${today}. از جستجوی زنده استفاده کن. پاسخ فارسی روان.`;
